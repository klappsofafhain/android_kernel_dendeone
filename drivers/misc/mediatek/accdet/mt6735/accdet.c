...
static enum hrtimer_restart accdet_timer_func(struct hrtimer *timer)
{
    ...
    switch (short_timer)
    {
        case 1:
            ...
            ACCDET_DEBUG("[accdet]KEY_PLAYPAUSE 0 \n");
            break;
        case 2:
            input_report_key(kpd_accdet_dev, KEY_NEXTSONG, 0);
            input_sync(kpd_accdet_dev);
            ACCDET_DEBUG("[accdet]KEY_NEXTSONG 0 \n");
            break;
        case 3:
            input_report_key(kpd_accdet_dev, KEY_PREVIOUSSONG, 0);
            input_sync(kpd_accdet_dev);
            ACCDET_DEBUG("[accdet]KEY_PREVIOUSSONG 0 \n");
            break;
        default:
            break;
    }
...
{
    ACCDET_DEBUG("[Accdet] hrtimer_init. \n");
    hrtimer_init(&accdet_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
    accdet_timer.function = accdet_timer_func;
}

static enum hrtimer_restart accdet_timer2_func(struct hrtimer *timer)
{
    ACCDET_DEBUG("[Accdet]accdet_timer2_func: accdet hook short press %d.\n", short_timer2);
    switch (short_timer2)
    {
        case 1:
            input_report_key(kpd_accdet_dev, KEY_PLAYPAUSE, 1);
            input_sync(kpd_accdet_dev);
            ACCDET_DEBUG("[accdet]KEY_PLAYPAUSE 1 \n");
            break;
        case 2:
            input_report_key(kpd_accdet_dev, KEY_NEXTSONG, 1);
            input_sync(kpd_accdet_dev);
            ACCDET_DEBUG("[accdet]KEY_NEXTSONG 1 \n");
            break;
        case 3:
            input_report_key(kpd_accdet_dev, KEY_PREVIOUSSONG, 1);
            input_sync(kpd_accdet_dev);
            ACCDET_DEBUG("[accdet]KEY_PREVIOUSSONG 1 \n");
            break;
        default:
            break;
    }
...
{
    ACCDET_DEBUG("[Accdet] hrtimer2_init. \n");
    hrtimer_init(&accdet_timer2, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
    accdet_timer2.function = accdet_timer2_func;
}
	...
	case MD_KEY:
		...
		// for press hook twice or three times 
         if (call_status == 0)
         {
             if (!flag) {
                 hrtimer_cancel(&accdet_timer);
                 ACCDET_DEBUG("[Accdet] hrtimer_cancel\n");
     
                 if (short_timer < 3)
                     short_timer++;
                 else
                 {
                     short_timer = short_timer - 2;
                     input_report_key(kpd_accdet_dev, KEY_PREVIOUSSONG, flag);
                     input_sync(kpd_accdet_dev);
                     ACCDET_DEBUG("[accdet]KEY_PREVIOUSSONG %d\n",flag);
                 }
                 ACCDET_DEBUG("[Accdet] hrtimer_start %d\n", short_timer);
                 hrtimer_start(&accdet_timer, ktime_set(0, 500*1000000), HRTIMER_MODE_REL);
             }
             else
             {
                 hrtimer_cancel(&accdet_timer2);
                 ACCDET_DEBUG("[Accdet] hrtimer2_cancel\n");
     
                 if (short_timer2 < 3)
                     short_timer2++;
                 else
                 {
                     short_timer2 = short_timer2 - 2;
                     input_report_key(kpd_accdet_dev, KEY_PREVIOUSSONG, flag);
                     input_sync(kpd_accdet_dev);
                     ACCDET_DEBUG("[accdet]KEY_PREVIOUSSONG %d\n",flag);
                 }
                 ACCDET_DEBUG("[Accdet] hrtimer_start %d\n", short_timer2);
                 hrtimer_start(&accdet_timer2, ktime_set(0, 500*1000000), HRTIMER_MODE_REL);
             }
         }
         else
         {
