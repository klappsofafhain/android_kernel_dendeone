...
//#ifdef ATMEL_DBG 
#define DBG(a,args...) printk("mxt %s %d " a, __func__, __LINE__, ##args)
//#endif

//#undef CONFIG_MXT_PLUGIN_SUPPORT

#define CONFIG_MXT_UPDATE_BY_OBJECT
#define WAKEUP_GESTURE_ENABLE

#ifdef WAKEUP_GESTURE_ENABLE
/* GESTURE */
...
#define CONFIG_ESD_CHECK_A_REGISTER 
#define CONFIG_TP_DEVICE_INFO_SUPORT
...
//frank undef for debug
//#undef CONFIG_ESD_CHECK_A_REGISTER 
...
#define LCSH_DEBUG(a, arg...) printk(TPD_DEVICE ":func=%s,line=%d, " a, __func__,__LINE__,##arg)
#else
...
//frank add
	unsigned int mxt_double_click_wake_enable;
...
const static unsigned int mxts_keys[NUM_KEY_TYPE][MAX_KEYS_SUPPORTED_IN_DRIVER] = {
	//T15_T97_KEY
	{KEY_MENU/*KEY_APP_SWITCH*/,KEY_HOMEPAGE,KEY_BACK,KEY_WAKEUP,KEY_F20},
	//T19_KEY,
	{KEY_F20},
	//T24_KEY,
	{KEY_F8},
...
//frank add
u8 *para_buf;
ssize_t touch_debug_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
...
static ssize_t mxt_plugin_glove_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t mxt_plugin_glove_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
static ssize_t mxt_plugin_stylus_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t mxt_plugin_stylus_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
static ssize_t mxt_plugin_wakeup_gesture_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t mxt_plugin_wakeup_gesture_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
static ssize_t mxt_plugin_gesture_list_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t mxt_plugin_gesture_list_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
static ssize_t mxt_plugin_gesture_trace_show(struct device *dev, struct device_attribute *attr, char *buf);
#	endif
#	if defined(CONFIG_MXT_MISC_WORKAROUND)
static ssize_t mxt_plugin_misc_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t mxt_plugin_misc_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
#	endif
#	if defined(CONFIG_MXT_CLIP_WORKAROUND)
static ssize_t mxt_plugin_clip_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t mxt_plugin_clip_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
#	endif
#endif
...
//static struct mxt_data  *waitful_data = NULL; 




static u8 suspend_flag = 0;
static void write_data_to_read(struct mxt_data *data); 
#endif
...
static int mxt_set_obj_cfg(struct mxt_data *data, struct reg_config *config, u8 *stack_buf);
static int mxt_unset_obj_cfg(struct mxt_data *data, struct reg_config *config, u8 *stack_buf);
...
//frank modify move from  atomic_op.h
static inline unsigned long test_flag(unsigned long mask, const volatile unsigned long *addr)
{
	return ((*addr) & mask) != 0;
}
...
static int mxt_read_lockdown_info(struct mxt_data *data)
{
	...
	/*DBG("Lockdown info: %02X %02X %02X %02X %02X %02X %02X %02X\n",
...
/* frank TBD */
#if 0
static int mxt_force_write_lockdown(struct mxt_data *data, unsigned char* buf, int length)
{
	struct device *dev = &data->client->dev;
	struct mxt_object *object;
...
static int mxt_load_suspend_para(struct mxt_data *data)
{
	int i;
	u8 *current_buf_position;
	
	current_buf_position = para_buf;

	for(i = 0; i < (sizeof(mxt_dwakeup_cfg)/sizeof(mxt_dwakeup_cfg[0])); i++) 
	{
		mxt_set_obj_cfg(data, &mxt_dwakeup_cfg[i], current_buf_position);
		current_buf_position += mxt_dwakeup_cfg[i].len;
	}
...
static int mxt_load_resume_para(struct mxt_data *data)
{
	int i;
	u8 *current_buf_position;
	...
	for(i = 0; i < (sizeof(mxt_dwakeup_cfg)/sizeof(mxt_dwakeup_cfg[0])); i++) 
	{
		mxt_unset_obj_cfg(data, &mxt_dwakeup_cfg[i], current_buf_position);

		current_buf_position += mxt_dwakeup_cfg[i].len;
	}
	/* just reset the chip to be ok */
	return 0;
}
...
static int mxt_set_obj_cfg(struct mxt_data *data, struct reg_config *config, u8 *stack_buf)
{
	...
	dev_info(dev, "mxt set t%d offset %d, len %d mask 0x%lx\n",
		config->reg,
		config->offset,
		config->len,
		config->mask);
	...
	if (config->len == 0) {
		dev_err(dev, "Skip zero request: reg %d off %d len %d\n", config->reg,config->offset,config->len);
		return 0;
	}
	...
	if (!object) {
		dev_err(dev, "Error: Object not found: reg %d off %d len %d\n",
			config->reg,config->offset,config->len);
		return -ENODATA;
	}
	if (!object->start_address) {
		dev_err(dev, "Error: Object address not found: reg %d off %d len %d\n",
			config->reg,config->offset,config->len);
		return -EINVAL;
	}
	if (object->instances_minus_one < config->instance) {
		dev_err(dev, "Error: Tried to write outside object T%d instance %d not exist", config->reg,config->instance);
		return -EINVAL;
	}
	if (config->offset >= mxt_obj_size(object)) {
		dev_err(dev, "Error: Tried to write outside object T%d" " offset:%d, size:%d\n", config->reg, config->offset, mxt_obj_size(object));
		return -EINVAL;
	}
	if (config->len > sizeof(old_buf)) {
		dev_err(dev, "Error: Config data too long: reg %d off %d len %d\n", config->reg,config->offset,config->len);
		return -EINVAL;
	}

	reg = object->start_address + mxt_obj_size(object) * config->instance + config->offset;

	//DBG("mxt set address %d\n",reg);

	if (stack_buf || config->mask) {
		ret = __mxt_read_reg(data->client, reg, config->len, old_buf);

		if (ret) {
			dev_err(dev, "Error: Config read reg failed: reg %d off %d len %d\n", config->reg,config->offset,config->len);
			return ret;
		}
...
//frank debug
	/*DBG("mxt set T%d offset %d, len %d mask 0x%lx\n",
			config->reg,
			config->offset,
			config->len,
			config->mask);
...
static int mxt_unset_obj_cfg(struct mxt_data *data, struct reg_config *config, u8 *stack_buf)
{
	...
	dev_info(dev, "mxt unset t%d offset %d, len %d mask 0x%lx\n",
		config->reg,
		config->offset,
		config->len,
		config->mask);
	...
	//DBG("mxt unset address %d\n",reg);

	ret = __mxt_write_reg(data->client, reg, config->len, buf);

	DBG("mxt unset T%d offset %d, len %d mask 0x%lx\n",
			config->reg,
			config->offset,
			config->len,
			config->mask);
...
#if defined(CONFIG_ESD_CHECK_A_REGISTER)
static int esd_check_function(struct mxt_data *data)  
{
        ...
        do
	{
               ...
               atmel_power_switch(SWITCH_OFF);
               tpd_gpio_output(GTP_RST_PORT, 0);	
               msleep(10);
           	   atmel_power_switch(SWITCH_ON);
               msleep(50);
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	if (status & MXT_T100_DETECT) {
		...
		input_report_abs(input_dev, ABS_MT_POSITION_X, x);
		input_report_abs(input_dev, ABS_MT_POSITION_Y, y);
//printk("swf999 X=%x,Y=%x,x=%x,y=%x\n",ABS_MT_POSITION_X,ABS_MT_POSITION_Y,x,y);
...
static void mxt_proc_t15_messages(struct mxt_data *data, u8 *msg)
{
	...
	printk("mxt swf99t15,line=%d,num_keys is %d.\n",__LINE__,num_keys);
	for (key = 0; key < num_keys; key++) {
		curr_state = test_bit(key, &data->t15_keystatus);
		new_state = test_bit(key, &keystates);

printk("mxt swf99t15,line=%d,curr_state is %d,new_state is %d.\n",__LINE__,curr_state,new_state);
...
#if 1//defined(CONFIG_MXT_PLUGIN_SUPPORT)
...
static void mxt_proc_T93_messages(struct mxt_data *data, u8 *msg)
{
	...
	if ((msg[1] & MXT_GESTURE_DOUBLE_CLICK_MASK) && (data->enable_wakeup)) {
		//DBG("\n");
		input_report_key(dev, KEY_F20, 1);
		input_sync(dev);
...
//pmic_set_register_value(PMIC_RG_VIBR_EN, 0);
		...
		input_report_key(dev, KEY_F20, 0);
		input_sync(dev);
		//data->enable_wakeup = 0;
	}
}
...
ssize_t touch_debug_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	switch(cmd)
	{
		...
		case 1:    
			...
			break;
		case 2:   
			input_report_key(input_dev, KEY_F20, 1);
			input_sync(input_dev);
			input_report_key(input_dev, KEY_F20, 0);
			input_sync(input_dev);
			break;
		case 3:   
			...
			break;
		case 4:
			mxt_load_suspend_para(data);
			break;
		case 5:
			mxt_load_resume_para(data);
			break;
		default:
			break;
	}
...
static ssize_t mxt_double_click_wake_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);

	return snprintf(buf, PAGE_SIZE, "enable_double_click_wakeup:%u\n", data->mxt_double_click_wake_enable);
}

static ssize_t mxt_double_click_wake_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
	struct mxt_data *data = dev_get_drvdata(dev);
	...
	data->mxt_double_click_wake_enable = input;
...
//frank modify
		...
		ret = mxt_proc_gesture_messages(data, MXT_PROCI_TOUCHSEQUENCELOGGER_T93,T93_KEY, msg);
		if (!ret)
			set_bit(MXT_WK_DETECTED,&data->enable_wakeup);
		else
...
static int mxt_proc_message(struct mxt_data *data, u8 *message)
{
	...
	//	printk("swf99t100 pos, id=%u, min=%u,mxt=%u\n",report_id,data->T100_reportid_min,data->T100_reportid_max);
...
printk("mxt swf99t97  id=%u, min=%u,mxt=%u,line=%d\n",report_id,data->T97_reportid_min,data->T97_reportid_max,__LINE__);
//printk(TPD_DEVICE ":func=%s,mxt swf99t97line=%d, " a, __func__,__LINE__,##arg)
...
static int mxt_process_message_thread(void *dev_id)
{
	...
	while (!kthread_should_stop()) {
		...
		dev_warn(&data->client->dev, "mxt_process_message_thread busy %lx suspend %d  boot %d interval %ld(0x%lx)\n",
			data->busy,
			data->suspended,
			data->in_bootloader,
			interval,interval);
...
static int mxt_check_reg_init(struct mxt_data *data)
{
	...
	if (info_crc == data->info_crc) {
		if (config_crc == 0 || data->config_crc == 0) {
			...
			//DBG("\n");
			dev_info(dev, "Config CRC in data->config_crc eaquals config_crc of file 0x%06X: OK\n", data->config_crc);
		...
		mxt_t6_command(data, MXT_COMMAND_CALIBRATE, 1, false);
			...
			//DBG("\n");
			dev_info(dev, "Config CRC in data->config_crc 0x%06X: does not match file 0x%06X\n", data->config_crc, config_crc);
		}
	} else {
		//DBG("\n");
...
static int mxt_set_t7_power_cfg(struct mxt_data *data, u8 sleep)
{
	...
	//DBG("sleep status: %d  deepsleep.active: %d deepsleep.idle: %d\n", sleep, deepsleep.active, deepsleep.idle);
	...
	else
	{
		...
		//DBG("sleep status: %d  newconfig->active: %d new_config->idle: %d\n", sleep, new_config->active, new_config->idle);
	}
...
recheck:
	...
	if (data->t7_cfg.active == 0 || data->t7_cfg.idle == 0) {
		if (!retry) {
			...
			//frank change this default value
...
static void mxt_regulator_enable(struct mxt_data *data)
{
	...
	if(ret)
	    printk("regulator_enable vdd error\n");

	ret = regulator_enable(data->reg_avdd);
	if(ret)
	    printk("regulator_enable avdd error\n");
...
static int mxt_update_file_name(struct device *dev, char **file_name,const char *buf, size_t count, int alternative)
{
	...
	*file_name = file_name_tmp;
	...
	printk("atmel upgrade_cfg count = %d \n",count);
	...
	dev_info(dev,"[mxt]mxt_update_cfg_store alternative is %d\n", alternative); 

	//DBG("mxt_update_cfg_store alternative is %d\n", alternative); 
	...
	if (alternative == 1) {
		...
		/* Truly */
		if((data->lockdown_info[0] == 0x31) && 
		   (data->lockdown_info[1] == 0x31) && 
		   (data->lockdown_info[2] == 0x31) && 
		   (data->lockdown_info[3] == 0x31) && 
		   (data->lockdown_info[4] == 0x45) && 
		   (data->lockdown_info[5] == 0xA2) && 
		   (data->lockdown_info[6] == 0x00) && 
                   (data->lockdown_info[7] == 0x00)){
			strncat((*file_name),".01",size);
		}

		/* Truly */
		if((data->lockdown_info[0] == 0x31) && 
		   (data->lockdown_info[1] == 0x31) && 
		   (data->lockdown_info[2] == 0x32) && 
		   (data->lockdown_info[3] == 0x31) && 
		   (data->lockdown_info[4] == 0x45) && 
		   (data->lockdown_info[5] == 0xA2) && 
		   (data->lockdown_info[6] == 0x00) && 
                   (data->lockdown_info[7] == 0x00)){
			strncat((*file_name),".02",size);
		}

		/* Goworld */
		if((data->lockdown_info[0] == 0x33) && 
		   (data->lockdown_info[1] == 0x34) && 
		   (data->lockdown_info[2] == 0x31) && 
		   (data->lockdown_info[3] == 0x31) && 
		   (data->lockdown_info[4] == 0x45) && 
		   (data->lockdown_info[5] == 0xA2) && 
		...
		if((data->lockdown_info[0] == 0x33) && 
		   ...
		   (data->lockdown_info[6] == 0x00) && 
                   (data->lockdown_info[7] == 0x00)){
			strncat((*file_name),".04",size);
		}


	}

	//DBG("mxt_update_cfg_store cfg name is %s\n", *file_name);
...
static ssize_t mxt_update_cfg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	int wait = 10;
	int ret;
...
//frank modify for bug.h compile error

static DEVICE_ATTR(devcfg_crc, 0644, mxt_devcfg_crc_show, NULL);//susan
...
static DEVICE_ATTR(debug_enable, 0644| S_IRUSR, mxt_debug_enable_show, mxt_debug_enable_store);
static DEVICE_ATTR(bootloader, 0644, mxt_bootloader_show, mxt_bootloader_store);
static DEVICE_ATTR(t19, 0644, mxt_t19_gpio_show, mxt_t19_gpio_store);
static DEVICE_ATTR(t25, 0644, mxt_t25_selftest_show, mxt_t25_selftest_store);
static DEVICE_ATTR(depth, 0644, mxt_irq_depth_show, mxt_irq_depth_store);
static DEVICE_ATTR(double_click_wake_enable, 0644, mxt_double_click_wake_enable_show, mxt_double_click_wake_enable_store);
static DEVICE_ATTR(touch_debug, 0644, NULL, touch_debug_store);


#if defined(CONFIG_MXT_PLUGIN_SUPPORT)
static DEVICE_ATTR(plugin, 0644,  mxt_plugin_show, mxt_plugin_store);
static DEVICE_ATTR(plugin_tag, 0644, mxt_plugin_tag_show, NULL);

#if defined(CONFIG_MXT_PROCI_PI_WORKAROUND)
static DEVICE_ATTR(en_glove, 0644,  mxt_plugin_glove_show, mxt_plugin_glove_store);
static DEVICE_ATTR(en_stylus, 0644, mxt_plugin_stylus_show,	mxt_plugin_stylus_store);
static DEVICE_ATTR(en_gesture, 0644, mxt_plugin_wakeup_gesture_show, mxt_plugin_wakeup_gesture_store);
static DEVICE_ATTR(gesture_list, 0644,  mxt_plugin_gesture_list_show, mxt_plugin_gesture_list_store);
static DEVICE_ATTR(gesture_trace, 0644, mxt_plugin_gesture_trace_show, NULL);
#endif
#if defined(CONFIG_MXT_MISC_WORKAROUND)
	static DEVICE_ATTR(misc, 0644, mxt_plugin_misc_show, mxt_plugin_misc_store);
#endif
#if defined(CONFIG_MXT_CLIP_WORKAROUND)
	static DEVICE_ATTR(clip, 0644, mxt_plugin_clip_show, mxt_plugin_clip_store);
#endif
#endif

#if defined(CONFIG_MXT_SELFCAP_TUNE)
static DEVICE_ATTR(self_tune, 0644, NULL, mxt_self_tune_store);
#endif
...
static void mxt_start(struct mxt_data *data, bool resume)
{
	...
	//frank add
	data->enable_wakeup = data->mxt_double_click_wake_enable;
	//DBG("data->enable_wakeup: %d\n",data->enable_wakeup);
...
//frank add
	...
	//DBG("data->enable_wakeup: %d\n",data->enable_wakeup);
#endif
	/* This function includes the diffrent setting for wakeup function */
...
//frank add
		if(data->enable_wakeup)
		{
			//DBG("mxt_load_suspend_para processing\n");
			mxt_load_suspend_para(data);
		}
...
#endif
	if (prop) {
printk("mxt swf99t15,line=%d\n",__LINE__);
		num_keys = devm_kzalloc(dev,sizeof(*num_keys) * NUM_KEY_TYPE, GFP_KERNEL);
			if (!num_keys){
				printk("mxt swf99t15,num_keys=%u,line=%d\n",num_keys,__LINE__);
				return NULL;
			}
		keymap = devm_kzalloc(dev,sizeof(*keymap) * NUM_KEY_TYPE, GFP_KERNEL);
		if (!keymap) {
		...
		num_keys[T93_KEY]=1;  
#else
		ret = of_property_read_u8_array(client->dev.of_node,"gpio-keymap-num", num_keys, NUM_KEY_TYPE);
		if (ret) {
			dev_err(dev,"Unable to read device tree key codes num keys: %d\n",ret);
			devm_kfree(dev,num_keys);
		...
		for (i = 0; i < NUM_KEY_TYPE; i++) {
			...
			if (ret) {
				dev_err(dev,"Unable to read device tree key codes: %d\n",ret);
				//return NULL;
				num_keys[i] = 0;
			}else {
	...
	if (pdata->num_keys) {
		for (i = 0; i < NUM_KEY_TYPE; i++) {
			...
			for (j = 0; j < pdata->num_keys[i]; j++) 
				dev_info(&client->dev, "%d ",pdata->keymap[i][j]);
			    dev_info(&client->dev, "\n");
printk("mxt type %d : ", pdata->keymap[i][j]);
		}
	}
...
static char *tp_vendor; //[20] = {1};
static char tp_version_temp[10]={0};// = "Goworld";//{0};
static char crc_info_compare[10] = "0xCA27CC";//"0xB15621"; //if TP is xinli   //"0xABC461"--->"Goworld"
...
static void tp_devinfo_init(struct mxt_data *data)
{
	static char crc_info[50] = {0};
        ...
        static char walker_info[50] = {0};
        int crc_temp;
	//static char *tp_buf=NULL;

       // tp_vendor = data->cfg_name;
	sprintf(crc_info, "0x%06X",data->config_crc);
	printk("swf77777 crc_info=0x%07X \n",data->config_crc);
        
        printk("zgyzgyzgy crc_info=0x%07X \n",data->config_crc);
...
if (!strncmp(tp_vendor, "A4_15_2.2_AA.raw.01", 19) & !strncmp(tp_vendor, "A4_15_2.2_AA.raw.02", 19) & !strncmp(tp_vendor, "A4_15_2.2_AA.raw.05", 19) & !strncmp(tp_vendor, "A4_15_2.2_AA.raw.06", 19))
	...
	sprintf(tp_version_temp, "%s","Goworld");  
        ...
        crc_temp = strncmp(crc_info,crc_info_compare,7);
        if(crc_temp == 0)
           sprintf(tp_version_temp, "%s","xinli");
        else
           sprintf(tp_version_temp, "%s","Goworld");
	...
	if(NULL != devinfo_tp)
	{
		...
		devinfo_tp->device_vendor = tp_version_temp;  //walker
...
static int tpd_irq_registration(struct mxt_data *data)
{
	...
	if (node) {
		...
		touch_irq = irq_of_parse_and_map(node, 0);
			ret = request_irq(touch_irq, mxt_interrupt,
			...
			touch_irq = irq_of_parse_and_map(node, 0);
			data->irq = touch_irq;
			ret = request_threaded_irq(touch_irq, NULL, mxt_interrupt_pulse_workqueue, //mxt_interrupt,
	...
	printk("[%s]irq:%d, debounce:%d-%d:", __func__, touch_irq, ints[0], ints[1]);
	return ret;
}

static int mxt_probe(struct i2c_client *client,	const struct i2c_device_id *id)	//__devinit mxt_probe 
{
	struct mxt_data *data;
	int error;
...
#if defined(CONFIG_ESD_CHECK_A_REGISTER)
       // waitful_data = kzalloc(sizeof(struct mxt_data), GFP_KERNEL);  
#endif
	if (!data) {
		dev_err(&client->dev, "Failed to allocate memory\n");
		return -ENOMEM;
	...
	/* frank add initalize mxt_double_click_wake_enable value */
	data->mxt_double_click_wake_enable = 0;

	para_buf = kzalloc(256, GFP_KERNEL);
	...
	if (!data->irq_tsk) {
       ...
       error = tpd_irq_registration(data);
	 client->irq = data->irq;
	printk("atmel data->irq = %d\n", data->irq);
...
#endif
#else
	/*error = request_threaded_irq(data->irq, NULL, mxt_interrupt_pulse_workqueue, //mxt_interrupt,
...
//#endif
	...
	if(proc_create("ctp_vender", 0444, NULL, &ctp_vender_fops) == NULL)
	{
                printk("create_proc_entry  failed\n");
		return -1;
        }	
...
//frank remove the interface.c and put the relevant functions here
#if 0
...
static int  tpd_probe(struct i2c_client *client, const struct i2c_device_id *id) //__devinit
{
	int ret;
...
{
	printk("Atmel MXT I2C Touchscreen Driver");
...
#if defined(CONFIG_ESD_CHECK_A_REGISTER)
	suspend_flag = 0;
    wake_up_interruptible(&esd_waiter);
#endif
