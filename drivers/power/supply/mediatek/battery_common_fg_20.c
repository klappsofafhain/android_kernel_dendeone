...
#if defined(BATTERY_WIRELESS_SUPPORT)  //add by Jason
...
extern bool charger_detect_flag; //add for G10.B-556//Jason.
#endif
#ifdef CONFIG_TCPC_CLASS
...
struct battery_data {
	...
	int BAT_batt_temp;
#if defined(CONFIG_FIH_PROJECT_FRT) || defined(CONFIG_FIH_PROJECT_NE1)
	//Jason---battery Charge full design---
...
#if defined(CONFIG_FIH_PROJECT_NE1)
	//Jason: power on value of voltage is 3.45V (3400 -> 3450)
	ret_value = 3450;
#else
	ret_value = 3400;
#endif
...
#if defined(CONFIG_FIH_PROJECT_NE1)
	//modify by Jason; (voltage accordance temp)
	//ret_value = 3400;
					 ...
					 char *buf)
{
	...
	extern unsigned short fih_hwid; //Jason.
						  ...
						  char *buf)
{
	...
	//modidy by Jason for battery current node
...
#if defined(CONFIG_FIH_PROJECT_NE1)
			//Jason: DUT do not show charging-icon at high temperature plugging usb.
...
/*add for shut down when Vbat<3.3v*/
	if(!(g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT))
	{
		...
		if((BMT_status.temperature < 0 && bat_vol < SYSTEM_SHUTDOWN_VOLTAGE_LOW_TEMP)//Jason: 3000mV.
			|| (BMT_status.temperature >= 0 && bat_vol < SYSTEM_SHUTDOWN_VOLTAGE))//Jason: 3300mV
	...
	if (bat_data->BAT_CAPACITY <= DLPT_POWER_OFF_THD) {
		...
		if (dlpt_check_power_off() == 1) {
			...
			if (cnt >= 4) {
				...
				//Jason. modify for battery temperature specification
				//kernel_restart("DLPT reboot system");
				//orderly_reboot(true); 

				bat_vol = battery_meter_get_battery_voltage(KAL_TRUE);				
				if (BMT_status.temperature < 0) {
					if (bat_vol < SYSTEM_SHUTDOWN_VOLTAGE_LOW_TEMP) {//Jason: 3000mV.
...
PMU_STATUS do_batt_temp_state_machine(void)
{
	...
	if (BMT_status.temperature == batt_cust_data.err_charge_temperature) {
		g_charging_error |= (1<<BAT_NTC_PIN_ERROR);	//Jason: charging error information.[20170111]
...
#if defined(CONFIG_FIH_PROJECT_FRT) || defined(CONFIG_FIH_PROJECT_NE1)
			g_charging_error |= (1<<BAT_LOW_TEMP_ERROR);	//Jason: charging error information.[20170111]
#endif
			return PMU_STATUS_FAIL;
...
#if defined(CONFIG_FIH_PROJECT_FRT) || defined(CONFIG_FIH_PROJECT_NE1)
		g_charging_error |= (1<<BAT_HIGH_TEMP_ERROR);	//Jason: charging error information.[20170111]
#endif
		return PMU_STATUS_FAIL;
...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		...
		if (BMT_status.charger_vol <= batt_cust_data.v_charger_min) {
			...
			BMT_status.bat_charging_state = CHR_ERROR;
#if defined(CONFIG_FIH_PROJECT_FRT) || defined(CONFIG_FIH_PROJECT_NE1)
			g_charging_error |= (1 << CHG_LOW_VOL_ERROR);	//Jason: charging error information.[20170111]
#endif
			status = PMU_STATUS_FAIL;
		}
#if defined(CONFIG_FIH_PROJECT_FRT) || defined(CONFIG_FIH_PROJECT_NE1)
		...
		if (BMT_status.charger_vol >= v_charger_max) {
			...
			BMT_status.bat_charging_state = CHR_ERROR;
#if defined(CONFIG_FIH_PROJECT_FRT) || defined(CONFIG_FIH_PROJECT_NE1)
			g_charging_error |= (1 << CHG_HIGH_VOL_ERROR);	//Jason: charging error information.[20170111]
#endif
			status = PMU_STATUS_FAIL;
		}
#if defined(CONFIG_FIH_PROJECT_FRT) || defined(CONFIG_FIH_PROJECT_NE1)
...
static void mt_battery_check_charge_time_limit(void)
{
	...
	char temp[8];
	static int flag = 0;
	static unsigned int count = 0;	//Jason: for test!
	...
	if (count || (g_battery_thermal_throttling_flag & 0x02)) //Jason: for test!
...
#if defined(CONFIG_FIH_PROJECT_NE1)
	/*Jason: control the switch of charging timer */
...
#if defined(CONFIG_FIH_PROJECT_NE1)
			BMT_status.bat_in_recharging_state = KAL_FALSE;//Jason: add for D78.B-798
#endif
#if defined(CONFIG_FIH_PROJECT_FRT) || defined(CONFIG_FIH_PROJECT_NE1)
			g_charging_error |= (1 << CHARGING_TIME_OVER_ERROR);	//Jason: charging error information.[20170111]
#endif
...
static PMU_STATUS mt_battery_CheckCallState(void)
{
	...
	if ((g_call_state == CALL_ACTIVE) && (BMT_status.bat_vol > V_CC2TOPOFF_THRES)) {
		status = PMU_STATUS_FAIL;
		g_charging_error |= (1 << CALLING_STATE_ERROR);	//Jason: charging error information.[20170111]
...
static void mt_battery_CheckBatteryStatus(void)
{
	...
	if (cmd_discharging == 1) {
		...
		BMT_status.bat_charging_state = CHR_ERROR;
#if defined(CONFIG_FIH_PROJECT_FRT) || defined(CONFIG_FIH_PROJECT_NE1)
		g_charging_error |= (1 << CMD_DISCHARGING_ERROR);	//Jason: charging error information.[20170111] 
#endif
		...
		if (BMT_status.temperature >= 60) {
...
#else
			{
				if ((g_platform_boot_mode == META_BOOT)
					...
					//battery_charging_control(CHARGING_CMD_SET_POWER_OFF, NULL);
					///* mt_power_off(); */
					if (BMT_status.charger_exist == KAL_TRUE) //Jason: power_off
						orderly_reboot(true);
					else
...
//modify by Jason for retry charger type
...
/*** End when cable out disable charging,20150717 ***/
	...
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
			...
			BMT_status.charger_exist = KAL_TRUE;
#if defined(CONFIG_FIH_PROJECT_NE1)
			charger_detect_flag = true; //Jason:add for G10.B-556
#endif
...
#if defined(CONFIG_FIH_PROJECT_NE1)
	Battery_CurrentConsumption();// Jason.
#endif
#if defined(CONFIG_FIH_PROJECT_FRT) || defined(CONFIG_FIH_PROJECT_NE1)
...
#if defined(CONFIG_FIH_PROJECT_NE1)
			orderly_reboot(true); //Jason. power_off.
#else
...
#if defined(CONFIG_FIH_PROJECT_FRT) || defined(CONFIG_FIH_PROJECT_NE1)
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Charger_Switch_Value);//add by Jason.
#endif
